  ...
  <div class="app-container">
    ...
    <tree-table :data="data"  border>
      ...
      <el-table-column label="图标" width="50">
        <template slot-scope="scope">
            <span><svg-icon :icon-class="scope.row.menuIcon" /> </span>      
        </template>
      </el-table-column>        
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        vueFile: [{ required: true, message: "Vue文件不能为空", trigger: "blur" }],
        vueUrl: [
          {
            required: true,
  ...
  methods: {
    ...
    modifyPermission(row) {
      ...
      if (!row.permission || row.permission.length == 0) {
        this.permissionCollection.push({ permissionCode: "", permissionName: "" });
      } else {
    ...
    pdel(index) {
      if (this.permissionCollection.length == 1) {
        this.permissionCollection = [{ permissionCode: "", permissionName: "" }];
      } else {
